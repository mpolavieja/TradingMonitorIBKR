...
from src.domain.custom_types import Position, Portfolio
...
class PortfolioTracker:
    ...
    def __init__(self):
        ...
        self.controlEstrategias = connectToGoogleSheets(
            retries=3,
            delay=1,
            sheetToTest="Maestro",
    ...
    def readInstrumentsFromGoogleSheets(
        self, sheetName: str = "Maestro", column: int = 5, startRow: int = 2
    ) -> list[str]:
        """
        Reads instrument symbols from column E of the Fichas Google Sheet.

        Args:
            sheetName: The name of the sheet/tab to read from (default: "Maestro")
            column: The column to read from (default: 4)
            startRow: The row to start reading from (default: 2, to skip header)

        Returns:
            List of unique instrument symbols (strings), cleaned and uppercased
        """
        instruments: set[str] = set()

        try:
            data = self.fichas.readGSDataRaw(
                sheetName=sheetName,
                beginColumn=column,
                beginRow=startRow,
                columns=1,
            ...
            if data:
                for row in data:
                    # Type guard: ensure row is a list with at least one element
                    if isinstance(row, list) and len(row) > 0 and row[0]:
                        symbol = str(row[0]).strip().upper()
                        if symbol:
                            instruments.add(symbol)
                logger.info(
                    f"Read {len(instruments)} instruments from Fichas sheet (column {column}, starting row {startRow})"
                )
        except Exception as e:
            logger.error(f"Error reading instruments from Fichas Google Sheet: {e}")
            return []

        result = sorted(list(instruments))
        logger.info(f"Total unique instruments from Google Sheets: {len(result)}")
        return result
    ...
    def updateMarkPrices(self, rtData: DataManager, close: bool = False) -> None:
        try:
            for _, instrument in self.instrumentDictionary.items():
                ...
                timeStr = (
                    marketData.timestamp.strftime("%Y-%m-%d %H:%M:%S")
                    if marketData
                    else "N/A"
                )
    ...
    def convertDictToList(self) -> List[List[Any]]:
        try:
            ...
            logger.error(
                f"Method convertDictToList. Error converting dictionary to list: {e}"
            )
            return []
    ...
    def create(
        ...
        try:
            ...
            for underlyingSymbol in underlyingSymbols:
                ...
                self.instrumentDictionary[underlyingSymbol] = createInstrument(
    ...
    def writeToGoogleSheets(self, dataList: List[List[Any]]) -> None:
        ...
        """
        self.controlEstrategias.writeDataToSheet(
            sheet_name="Maestro", data=dataList, start_column="G", start_row=1
        )
        self.fichas.writeDataToSheet(
            sheet_name="Maestro", data=dataList, start_column="G", start_row=1
        )
